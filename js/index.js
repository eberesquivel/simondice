...
function checkMove(id) {
  ...
  var playerMove = colorRef[id]
    if (playerMove == color[index]){
      ...
      index = 0;
      countElement.innerHTML = 'Incorrecto. Intenta de nuevo';
      playColors();
    } else {
      countElement.innerHTML = 'Incorrect. Termino eñ inicio :-(';
      index = 0;
      startGame();
    }
  if (index == color.length) {
      ...
      movesCount++;
      movesLeft = movesCount;
      countElement.innerHTML = 'Siguiente nivel!'
      nextRound();
    }
}
...
function nextRound() {
  ...
  // and call playColors();
  if (movesCount > 20) {
    countElement.innerHTML = 'Ganaste! Felicidades!'
